...
class YunxinSmsApi extends Model{
     ...
     * @param string $needstatus 	是否需要状态报告22

     */

    public function sendSMS( $mobile,$captcha,$mid,$account,$key) {
        $account=$account;
        $key=$key;
        $mid=$mid;
        $key=md5($key);
        $url="https://ux.hk.cn/user/SsgApi.php?act=msgsend&account=$account&key=$key&m_id=$mid&mobile=$mobile&code=$captcha";
...
//        print_r($res);die;
    ...
    public function sendSMSa( $mobile, $msg, $method, $isLongSms = 0, $extenno='' ) {
